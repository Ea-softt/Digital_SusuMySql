...
import { initializePaystackPayment, initiateWithdrawal } from '../services/paystackService';
...
export const MemberDashboard: React.FC<MemberDashboardProps> = ({ group, transactions, userId, onRefresh, currentUser, members }) => {
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
      ...
      try {
          ...
          const newTx: Transaction = {
              id: result.transfer_code || `tx-w-${Date.now()}`,
              userId: currentUser.id,
              userName: currentUser.name,
              type: 'WITHDRAWAL',
              amount: amount,
          ...
          setWithdrawAmount('');
          setWithdrawPassword('');
          alert(`Successfully withdrew ${currency} ${amount} via Paystack.`);
      } catch (err) {
